...
library EvmFeeParamsLib {
  ...
  //  4 bytes pricePerByte - Mwei / byte
  ...
  uint256 private constant PRICE_PER_BYTE_SIZE = PricePerByteLib.BYTE_SIZE * 8;
  ...
  function pricePerByte(EvmFeeParams params) internal pure returns (PricePerByte) { unchecked {
    return PricePerByte.wrap(
      uint32(EvmFeeParams.unwrap(params) >> PRICE_PER_BYTE_OFFSET)
    );
  }}

  function pricePerByte(
    EvmFeeParams params,
    PricePerByte pricePerByte_
  ) internal pure returns (EvmFeeParams) { unchecked {
    return EvmFeeParams.wrap(
      (EvmFeeParams.unwrap(params) & PRICE_PER_BYTE_WRITE_MASK) |
      (uint256(PricePerByte.unwrap(pricePerByte_)) << PRICE_PER_BYTE_OFFSET)
    );
  }}
}
